...
// se solapan con el mes indicado (day_index === 0 = Domingo), más el costo fijo
// mensual de los líderes sin turnos (is_fixed_cost=true, columna costo_fijo_mensual).
export async function GET(request: NextRequest) {
  ...
  if (activeSchedules.length === 0) {
    // Aun sin cronogramas puede haber líderes fijos que sumar
    const emptyLeaderResult = await computeFixedLeaders(sb, area, weeks.length, new Map());
    return NextResponse.json({
      month,
      area,
      total_costo_empresa: emptyLeaderResult.leaderCosto,
      total_personas: emptyLeaderResult.leaderEntries.length,
      semanas: [],
      empleados: emptyLeaderResult.leaderEntries,
      lideres_fijos: emptyLeaderResult.leaderEntries,
      _debug: { weeks: weeks.length, schedules: 0, assignments: 0, leader_entries: emptyLeaderResult.leaderEntries.length },
    });
  }
  ...
  // Agregar turnos (calcularCostoTurnoEmpresa sanitiza el salario internamente)
  ...
  for (const a of (assignments || []) as { schedule_id: string; employee_id: string; day_index: number; shift_code: string }[]) {
    ...
    // Salario real del empleado; calcularCostoTurnoEmpresa lo sanitiza internamente.
    const isSunday = a.day_index === SUNDAY_DAY_INDEX;
    const costo = calcularCostoTurnoEmpresa(shiftType, salaryMap.get(a.employee_id) || 0, isSunday).total;
  ...
  // === LÍDERES DE COSTO FIJO (sin turnos) ===
  // Empleados activos con is_fixed_cost=true que NO tienen asignaciones de turno.
  // Su costo del mes = costo_fijo_mensual × (semanas del mes / 4.33).
  const fixedResult = await computeFixedLeaders(sb, area, weeks.length, empleadoMap);
  totalCosto += fixedResult.leaderCosto;
  ...
  const empleados = Array.from(empleadoMap.values())
    ...
    .concat(fixedResult.leaderEntries.map(e => ({ ...e, costo: Math.round(e.costo) })));

  return NextResponse.json({
    month,
    area,
    total_costo_empresa: Math.round(totalCosto),
    total_personas: totalPersonas + fixedResult.leaderEntries.length,
    semanas,
    empleados,
    lideres_fijos: fixedResult.leaderEntries.map(e => ({ ...e, costo: Math.round(e.costo) })),
    _debug: {
      weeks: weeks.length,
      schedules: activeSchedules.length,
      assignments: (assignments || []).length,
      skipped_no_type: skippedNoType,
      leader_entries: fixedResult.leaderEntries.length,
    },
  });
}

// Calcula el costo fijo mensual proporcional de los líderes is_fixed_cost=true
// que NO aparecen en empleadoMap (ya contados via turnos). Respeta el filtro de area.
async function computeFixedLeaders(
  sb: ReturnType<typeof getServiceClient>,
  area: string,
  semanasEnMes: number,
  empleadoMap: Map<string, EmpleadoAgg>
): Promise<{ leaderEntries: EmpleadoAgg[]; leaderCosto: number }> {
  let staffQuery = sb
    .from('pos_nomina_staff')
    .select('id, nombre_completo, cargo, area, salario, is_fixed_cost, costo_fijo_mensual, is_leader, rubro')
    .eq('activo', true)
  ...
  for (const s of (fixedRows || []) as {
    id: string; nombre_completo: string; cargo: string; area: string;
    salario: number | null; is_fixed_cost: boolean; costo_fijo_mensual: number | null;
    is_leader: boolean; rubro: string | null;
  }[]) {
    // Si el líder ya tiene turnos asignados, su costo ya está en empleadoMap
  ...
  return { leaderEntries, leaderCosto };
}