...
export async function GET(request: NextRequest) {
  ...
  // Also fetch completed/cancelled/no_show for the day
  ...
  for (const table of allTables) {
    const zId = (table.table_zones as { id: string; name: string } | null)?.id || 'unassigned'
    const zName = (table.table_zones as { id: string; name: string } | null)?.name || 'Sin zona'
    if (!zoneMap.has(zId)) {
