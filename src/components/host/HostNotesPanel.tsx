...
/**
 * Mobile-optimized notes panel for the host view.
 * Two sections: quick textarea for internal notes + threaded notes history.
 * Designed for touch: big buttons, clear labels, visual hierarchy.
 */
export function HostNotesPanel({ reservationId, internalNotes, onNotesUpdate, className }: HostNotesPanelProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/reservation-notes?reservation_id=${reservationId}`)
      ...
      .catch((err) => {
        console.error('[HostNotes] Failed to load notes:', err)
        setError('Error al cargar notas')
        setLoading(false)
      })
  }, [reservationId, showHistory])
  ...
  const handleAddNote = useCallback(async () => {
    ...
    try {
      ...
      console.error('[HostNotes] Failed to add note:', err)
  ...
  const handleSaveInternalNotes = useCallback(async () => {
    ...
    try {
      ...
      console.error('[HostNotes] Failed to save internal notes:', err)
      ...
      <div className="p-3">
        ...
        >
          ...
          <span className="text-[10px] text-[var(--color-ak-borgona)] dark:text-[var(--color-ak-dorado)] flex items-center gap-0.5">
            {showHistory ? 'Ocultar' : 'Ver notas'}
            {showHistory ? <CaretUp size={10} /> : <CaretDown size={10} />}
          </span>
        </button>
